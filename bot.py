...
# --- 2. Health Check Section (For Koyeb) ---
web_app = Flask(__name__)
@web_app.route('/')
def home(): return "1080p Bot is running!"
...
async def text_handler(_, msg):
    ...
    if text.startswith("http"):
        ...
        # 1080p ခလုတ်ကို ဤနေရာတွင် ထည့်သွင်းထားပါသည်
...
async def resolution_handler(_, cq):
    ...
    try:
        ...
        if subs:
            ...
            subtitle_state[user_id] = {"title": title, "lines": lines}
            await cq.message.reply_document(srt_path, caption=f"📄 Subtitle ({lang})")
            await cq.message.reply(f"✅ Subtitle ရပါပြီ\nစာကြောင်းရေ: ({len(lines)})\n\nဘယ်နှစ်ပိုင်း ခွဲမလဲ? (ဂဏန်းပို့ပါ)")
